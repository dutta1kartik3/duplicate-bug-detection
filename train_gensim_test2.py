...
def get_model(dimensions, name):
	...
	skipgram = Word2Vec(sg=1, size=dimensions, negative=5, min_count=5, workers=80)
...
def train_model(model, sentences, dimensions, algorithm):
	...
	while(epoch < EPOCHS):
		...
		if epoch%2 == 0:
			model.save(MODEL_DIR + "/test" + algorithm + "_model_dimensions_"+str(dimensions)+"_epoch_"+str(epoch)+".word2vec")
		epoch += 1
