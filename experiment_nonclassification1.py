...
def infer_bug_vector(words, dimensions, model):
...
def val(algorithm, dimensions, test_filename, epoch):
	...
	for line in test_file:
		try:
			...
			slave_vec = array(infer_bug_vector(line[1].split(" ") +   line[2].split(" "), int(dimensions), model))
			slave = matutils.unitvec(slave_vec).astype(REAL)
			master_vec = array(infer_bug_vector(line[5].split(" ") + line[6].split(" "), int(dimensions), model))
			master = matutils.unitvec(master_vec).astype(REAL)
			actual_labels.append(int(float(line[3])))
			actual_labels.append(int(float(line[3])))
