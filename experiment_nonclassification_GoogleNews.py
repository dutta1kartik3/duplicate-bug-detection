...
def val(algorithm, dimensions, test_filename, epoch):
	...
	for line in test_file:
		try:
			...
			slave_vec = array(infer_bug_vector(line[2].split(" ") +   line[3].split(" "), int(dimensions), model))
			slave = matutils.unitvec(slave_vec).astype(REAL)
			master_vec = array(infer_bug_vector(line[6].split(" ") + line[7].split(" "), int(dimensions), model))
	...
	result = "GoogleNews\t" +str(dimensions) + "\t" + str(epoch)+ "\t" + " ".join(accuracy)+"\n"
...
def __main__():
	...
	dimensions = argv[2]
	test_file = argv[3]
	val(algorithm, dimensions, test_file, 1)
