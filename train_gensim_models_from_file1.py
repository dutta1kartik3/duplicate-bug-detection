...
def get_model(dimensions, name):
	...
	dbow = Doc2Vec(dm=0, size=dimensions, negative=5, min_count=2, workers=50, dbow_words = 1)
        fastskipgram = FastText(sg=1, size=dimensions, negative=5, min_count=2, workers=100, hs =1 )
	fastcbow =  FastText(sg=0, size=dimensions, negative=5, min_count=2,workers=100, hs = 1)

	if name == "fastskipgram":
		return fastskipgram
	if name == "fastcbow":
		return fastcbow
	if (name == 'skipgram'):
...
def get_sent(filename):
	...
	return sentences
def __main__():
	algorithms = ['skipgram', 'cbow', 'pvdm', 'dbow', 'fastskipgram', 'fastcbow'] #'glove', 'varembed', 'wordrank']
	algorithm = argv[1]
