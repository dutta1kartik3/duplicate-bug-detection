...
def get_model(dimensions, name):
	...
	dbow = Doc2Vec(dm=0, size=dimensions, negative=5, min_count=2, workers=50, dbow_words = 1)
        fastskipgram = Word2Vec(sg=1, size=dimensions, negative=5, min_count=2, workers=100, hs =1 )
	fastcbow =  Word2Vec(sg=0, size=dimensions, negative=5, min_count=2,workers=100, hs = 1)
